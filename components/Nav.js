...
export const navData = [
  ...
  { name: 'categories', path: '/categories', icon: <HiSquares2X2 /> },
  { name: 'courses', path: '/courses', icon: <HiViewColumns /> },
 ...
 { name: 'NFT', path: '/claim', icon: <HiStar /> },
  {
    name: 'contact',
    path: '/contact',
    icon: <HiEnvelope />,
...
const Nav = () => {
  ...
  xl:right-[2%]  z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen '>
    ...
    <div className='flex w-full xl:flex-col items-center justify-between xl:justify-center
     gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] bg-red-200/10 xl:h-max py-8 bg-white/10 backdrop-blur-sm text-3xl
     xl:text-xl xl:rounded-full'>
      {navData.map((link, index)=> {
        return (
        <Link className={ `${link.path === pathname && 'text-accent'} relative flex items-center group
        hover:text-accent transition-all duratio-300`} 
